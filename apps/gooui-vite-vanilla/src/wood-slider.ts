...
export const woodMaterial = (ctx: MaterialContext): d.v3f => {
  ...
  const disp = d.vec2f(perlin2d.sample(dispUv), perlin2d.sample(dispUv.add(d.vec2f(100)))).mul(1);
  const ringLayer = std.sin(std.length(uv2.add(disp)) * 5) * 0.5 + 0.5;

  const albedo = d.vec3f(std.mix(dark, light, std.saturate(ringLayer * 0.8 + base * 0.5))).mul(0.7);
