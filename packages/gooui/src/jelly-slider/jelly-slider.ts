...
export class JellySlider {
  ...
  #createBindGroups() {
    return {
      rayMarch: this.root.createBindGroup(rayMarchLayout, {
        backgroundTexture: this.#backgroundTexture.sampled,
      }),
      render: [0, 1].map((frame) =>
        this.root.createBindGroup(sampleLayout, {
          currentTexture: this.#taaResolver.getResolvedTexture(frame),
        }),
      ),
    };
  }
  ...
  update(deltaSeconds: number): void {
    this.#slider.update(deltaSeconds);
  }

  render(targetView: GPUTextureView, width: number, height: number): void {
    this.handleResize(width * this.#qualityScale, height * this.#qualityScale);
...
const getSliderBbox = () => {
  ...
  return SdfBbox({
    ...
    bottom: d.f32(sliderBBoxSlot.$[2]),
    top: d.f32(sliderBBoxSlot.$[0]),
  });
};
...
const renderBackground = (
  ...
  const lightDir = lightAccess.$.direction;
...
const raymarchFn = tgpu["~unstable"].fragmentFn({
  ...
  randf.seed2(taaPrimerAccess.$.mul(uv));
